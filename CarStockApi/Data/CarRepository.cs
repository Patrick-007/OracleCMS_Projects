...
namespace CarStockApi.Data{
    public class CarRepository{
        ...
        {
            if (!_dealerStockList.ContainsKey(dealerId))
            {
                throw new KeyNotFoundException($"Dealer with ID {dealerId} not found.");
            }

            return _dealerStockList[dealerId];
        }
        ...
        public Car GetById(int dealerId, int carId){
            ...
            var car = _dealerCars[dealerId].FirstOrDefault(car => car.Car_ID == carId);
            if (car == null)
                throw new KeyNotFoundException($"Car with ID {carId} not found for dealer {dealerId}.");
        ...
        public void Add(int dealerId, Car car)
        {
            ...
            if (_dealerCars[dealerId].Any(c => c.Car_ID == car.Car_ID))
            {
                throw new InvalidOperationException($"Car with ID {car.Car_ID} already exists for dealer {dealerId}.");
            }
        ...
        public void Remove(int dealerId, Car car){
            ...
            if (existingCar == null)
                throw new KeyNotFoundException($"Car with ID {car.Car_ID} not found for dealer {dealerId}.");
        ...
        public void Update(int dealerId, int carId, Car car){
            if (!_dealerCars.ContainsKey(dealerId))
                throw new KeyNotFoundException($"Dealer with ID {dealerId} not found.");

            var existingCar = _dealerCars[dealerId].FirstOrDefault(car => car.Car_ID == carId);
            if (existingCar == null)
                throw new KeyNotFoundException($"Car with ID {carId} not found for dealer {dealerId}.");
        ...
        public IEnumerable<Car> Search(int dealerId, string make, string model){
            ...
            var cars = _dealerCars[dealerId].Where(car => car.Make.ToLower().Contains(make.ToLower()) && car.Model.ToLower().Contains(model.ToLower()));
            if (!cars.Any())
                throw new KeyNotFoundException($"No cars found matching make '{make}' and model '{model}' for dealer {dealerId}.");
